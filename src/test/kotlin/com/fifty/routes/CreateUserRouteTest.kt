...
import com.fifty.di.testModule
import com.fifty.plugins.configureSerialization
...
internal class CreateUserRouteTest : KoinTest {
    ...
    @Test
    fun `CreateUser, no body attached, responds with BadRequest`() {
        withTestApplication(
            moduleFunction = {
                install(Routing) {
    ...
    fun `Create user, user already exists, responds with unsuccessful`() = runBlocking {
        ...
        withTestApplication(
            ...
            Truth.assertThat(response.message).isEqualTo(ApiResponseMessages.USER_ALREADY_EXISTS)
        }
    }
    ...
    fun `Create user, email is empty, responds with unsuccessful`() = runBlocking {
        withTestApplication(
            ...
            Truth.assertThat(response.successful).isFalse()
            Truth.assertThat(response.message).isEqualTo(ApiResponseMessages.FIELDS_BLANK)
        }
    }
    ...
    fun `Create user, valid data, responds with successful`() {
        withTestApplication(
            ...
            Truth.assertThat(response.successful).isTrue()

            runBlocking {
                val isUserInDb = userRepository.getUserByEmail("test@test.com") != null
                Truth.assertThat(isUserInDb).isTrue()
            }
        }
    }
}